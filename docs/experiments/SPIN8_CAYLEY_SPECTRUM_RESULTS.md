...
This is a theorem over the full orthonormal balanced information family, not a numerical
fit.
...
The block certificate now also checks the flag reduction's missing algebraic
step: for both chiral views, an arbitrary orthogonal basis change within a
same-view probe pair leaves the summed information projector exactly
unchanged. Thus the operator depends on the labelled pair of two-planes, not
on an accidental choice of basis within either plane.

A later exact isotropy audit closes the internal-split gap. At five rational
non-endpoint representatives, including the Cayley-null point, the four-plane
stabilizer has dimension 6 and its restriction spans all 6 dimensions of
`so(4)`; preserving the `2+2` split leaves dimension 2. The resulting flag
orbit has dimension 19 inside the 20-dimensional balanced flag space, so the
local quotient is one-dimensional. At both oriented Cayley endpoints the plane
stabilizer grows to dimension 9, but its restriction still spans `so(4)`, so
no unrecorded split invariant appears there either. The global statement that
the Cayley coordinate separates all four-plane orbits is the cited classical
cohomogeneity-one theorem, not a consequence of this finite rank audit alone.
This exact certificate is
[`spin8_cayley_flag.py`](../../src/spin8_cayley_flag.py); the formal quotient
argument is given in the
[`publication manuscript`](../manuscripts/CAYLEY_INFORMATION_SPECTRUM.md).
...
## Historical next mathematical gate

This was the next gate at the time of the 2026-08-03 result. Later exact work
proved the signed-star, Cayley-null edge, and variable-Cayley one-edge families,
but did not settle the unrestricted inequality described here.
...
The exact characteristic polynomial has now yielded three additional spectral
laws. Writing (z=c^2), exact logarithmic differentiation gives
...
Both are strictly increasing on (0\le z<1), while
...
are constant. Thus the Cayley-null orbit is simultaneously the unique
unoriented D-optimum, A-optimum, and inverse-Frobenius optimum within the
orthonormal balanced family. This strengthens the orbit theorem without
changing its scope: global optimality over nonorthogonal frames and other
allocations remains open.

The same block factors determine the singular approach exactly. The three
eigenvalues lost at (z=1)—one in each eight-dimensional block—share the
first-order law
...
Thus rank loss is first-order and has the same leading rate in the three
failing blocks,
while the spectral condition number grows as
...
[`spin8_cayley_criteria.py`](../../src/spin8_cayley_criteria.py), its artifact is
